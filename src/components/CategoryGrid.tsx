...
const CategoryGrid: React.FC<CategoryGridProps> = ({ groups, sortMode, onSortModeChange }) => {
  ...
  const toggleContentExpansion = (eventId: string) => {
                  ...
                  <div className="absolute inset-0 overflow-y-auto scrollbar-hide p-2 space-y-1.5">
                    <AnimatePresence mode="popLayout">
                      {group.events.map((event, idx) => {
                        const isExpanded = idx === 0 || expandedEventIds.has(event.id);
                        const isExpandable = idx !== 0 && Boolean(event.content);
                        ...
                        return (
                          <motion.div
                            ...
                            onClick={() => {
                              if (isExpandable) {
                                toggleContentExpansion(event.id);
                              }
                            }}
